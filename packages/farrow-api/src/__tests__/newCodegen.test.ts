...
import { Api } from '../api'
import { toJSON } from '../newToJSON'
import { codegen } from '../newCodegen'
...
describe('codegen', () => {
  it('support codegen', async () => {
    ...
    let expected = await fs.readFile(`${__dirname}/newExpected/01.ts`)
  ...
  it('can disable emiting api-client', async () => {
    ...
    let expected = await fs.readFile(`${__dirname}/newExpected/02.ts`)
